...
export const getRestaurants = async(limitRestaurants, startRestaurant, pagination) => {
    const result = { statusResponse: true, error: null, restaurants: [], startRestaurant: null }
    try {
        if (startRestaurant == null && pagination == true) { return result }
        const response = pagination ? (
            await db
            .collection("restaurants")
            .orderBy("createAt", "desc")
            .startAfter(startRestaurant.createAt)
        ...
        if (response.docs.length > 0) {
            result.startRestaurant = response.docs[response.docs.length - 1].data()
        }
